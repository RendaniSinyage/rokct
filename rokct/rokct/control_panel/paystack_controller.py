...
class PaystackController:
    ...
    def charge_customer(self, customer_email, amount_in_base_unit, currency="USD"):
        ...
        :param customer_email: The email of the customer to charge.
        :param amount_in_base_unit: The amount to charge in the base currency unit (e.g., dollars).
        :param currency: The currency of the charge (e.g., "USD").
        :return: A dictionary with 'success' (boolean) and a 'message'.
        """
        if not self.secret_key:
            return {"success": False, "message": "Paystack secret key is not configured."}

        # Assumption: The Paystack authorization code is stored on the Customer doctype.
        # This field will need to be created on the Customer doctype.
        customer = frappe.get_doc("Customer", {"customer_primary_email": customer_email})
        if not customer:
            return {"success": False, "message": f"Customer with email {customer_email} not found."}
        ...
        # Paystack requires the amount in the smallest currency unit (e.g., kobo/cents).
